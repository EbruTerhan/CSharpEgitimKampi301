...
namespace CSharpEgitimKampi301.EFProject
{
    public partial class FrmStatistics : Form
    {
        ...
        private void FrmStatistics_Load(object sender, EventArgs e)
        {
            ...
            lblLastCountryName.Text = db.Location.Where(x => x.LocationId == lastCounrtyId).Select(y => y.Country).FirstOrDefault();
            ...
            var guideIdByNerimanOzturk = db.Guide.Where(x => x.GuideName == "Neriman" && x.GuideSurname =="Öztürk").Select(y => y.GuideId).FirstOrDefault();
            lblNerimanOzturkLocationCount.Text = db.Location.Where(x => x.GuideId == guideIdByNerimanOzturk).Count().ToString();
        }
